...
import { createDraftQuotation, updateDraftQuotation } from "@/lib/quotations/actions";
import {
  calculateLineTotals,
...
type QuotationFormProps = {
  ...
  quotation?: QuotationFormQuotation;
};

export function QuotationForm({ businesses, customers, quotation }: QuotationFormProps) {
  const isEditing = Boolean(quotation);
  const [selectedBusinessId, setSelectedBusinessId] = useState(quotation?.businessProfileId ?? businesses[0]?.id ?? "");
  const [lineItems, setLineItems] = useState<QuotationFormLineItem[]>(() =>
  ...
  const action = quotation ? updateDraftQuotation.bind(null, quotation.id) : createDraftQuotation;
  const previewTotals = useMemo(() => calculatePreviewTotals(lineItems, currencyCode), [lineItems, currencyCode]);

  function updateLineItem(id: string, field: keyof Omit<QuotationFormLineItem, "id">, value: string) {
    ...
    <form action={action} className="space-y-8">
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Field defaultValue={formatDateForInput(quotation?.issueDate) ?? todayInputValue()} label="Issue date" name="issueDate" required type="date" />
          <Field defaultValue={formatDateForInput(quotation?.expiryDate)} label="Expiry date" name="expiryDate" type="date" />
        </div>

        <div className="mt-4 grid gap-4 md:grid-cols-2">
          <TextArea defaultValue={quotation?.notes ?? undefined} label="Notes" name="notes" />
          <TextArea defaultValue={quotation?.terms ?? undefined} label="Terms" name="terms" />
        </div>
      </section>
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600">Draft quotation totals are recalculated again on the server when saved.</p>
          </div>
          <button
      ...
      <section className="flex flex-col gap-4 rounded-3xl border border-slate-200 bg-slate-950 p-6 text-white sm:flex-row sm:items-center sm:justify-between">
        ...
        <button className="rounded-2xl bg-white px-5 py-3 text-sm font-semibold text-slate-950 transition hover:bg-slate-100" type="submit">
          {isEditing ? "Save draft quotation" : "Create draft quotation"}
        </button>
      </section>
    </form>
  );
...
function createEmptyLineItem(): QuotationFormLineItem {
  return {
...
function calculatePreviewTotals(lineItems: QuotationFormLineItem[], currencyCode: string) {
